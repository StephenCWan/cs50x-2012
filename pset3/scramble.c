...
{
    int s = 0;
    int e = dictionary.size;
    while (s <= e)
    {
        int mid = (e + s) / 2;
        if (strcmp(dictionary.words[mid].letters, str) > 0)
            e = mid - 1;
        else if (strcmp(dictionary.words[mid].letters, str) < 0)
            s = mid + 1;
        else if (!dictionary.words[mid].found)
        {
            // found
...
void scramble(void)
{
    ...
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int o = 0; o < DIMENSION; o++)
        {
            newgrid[i][DIMENSION - o - 1] = grid[o][i];
        }
    }
